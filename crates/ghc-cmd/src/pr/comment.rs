...
impl CommentArgs {
    ...
    pub async fn run(&self, factory: &crate::factory::Factory) -> Result<()> {
        ...
        let comment_body = match (&self.body, body_from_file, self.editor) {
            (Some(b), _, _) => b.clone(),
            (None, Some(b), _) => b,
            (None, None, true) => {
    ...
    /// Find the last comment by the authenticated user on the PR.
    ...
    /// Handle --edit-last: edit the user's last comment on the PR.
    async fn handle_edit_last(
        ...
        let Some((comment_id, existing_body, _html_url)) = last_comment else {
            ...
            anyhow::bail!(
                "no comments found by the current user on pull request #{}",
                self.number,
            );
        };
        ...
        ios_eprintln!(
            ios,
            "{} Edited comment on pull request #{}",
            cs.success_icon(),
            self.number,
        );
    ...
    /// Handle --delete-last: delete the user's last comment on the PR.
    async fn handle_delete_last(
        ...
        ios_eprintln!(
            ios,
            "{} Deleted comment on pull request #{}",
            cs.success_icon(),
            self.number,
        );
...
mod tests {
    ...
    async fn test_should_add_comment_to_pull_request() {
        ...
        let args = default_args(12, "owner/repo");
    ...
    async fn test_should_comment_and_open_in_browser() {
        ...
        let mut args = default_args(13, "owner/repo");
        args.web = true;
        args.run(&h.factory).await.unwrap();
