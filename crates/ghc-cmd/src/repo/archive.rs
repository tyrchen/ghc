...
pub struct ArchiveArgs {
    /// Repository to archive (OWNER/REPO).
...
impl ArchiveArgs {
    ...
    pub async fn run(&self, factory: &crate::factory::Factory) -> Result<()> {
        ...
        // Fetch repository to check if already archived and get node ID
        ...
        if !self.yes {
            let confirmed = factory
                .prompter()
                .confirm(&format!("Archive {full_name}?"), false)?;
            if !confirmed {
                return Ok(());
            }
        }
        ...
        let _: Value = client
            .graphql(ARCHIVE_MUTATION, &mutation_vars)
            .await
...
mod tests {
    ...
    #[tokio::test]
    async fn test_should_archive_repository() {
